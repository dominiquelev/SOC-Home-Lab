...
## 📌 1. Project Objective
the objective of this lab was to learn fundamental of network traffic analysis using Wireshark in Kali linux Virtual Machine environment

focused on:
- installed wireshark on VirtualBox
- configuring Wireshark correctly on VirtualBox
- understanding NAT vs Bridge Adapter networking for network traffic analysis
...
* **The Problem:**
NAT mode restricted the ability to observe live local network traffic properly. based on research and beginner Wireshark learning recommendations, Bridged Adapter mode provides better visibility for real-time packet analysis because the virtual machine connects more directly to the local network.
* **The Resolution Workflow:** 
  ...
  9. Applied the `icmp` display filter in Wireshark to verify that Wireshark successfully captured the ICMP traffic generated by the ping requests.
